...
def decrypt(data,key,nonce):
    ...
    return bytes(out)
relay=Path(sys.argv[1]); out=Path(sys.argv[2]); key=base64.b64decode(sys.argv[3])
if len(key) != 32: raise RuntimeError("invalid relay key length")
out.mkdir(parents=True,exist_ok=True)
writes=0
for chunk in range(8):
    nonce=bytes([0x51,0x50,0x56,0x31,0x35,0x31,0x43,0x48,chunk & 0xff,(chunk >> 8) & 0xff,0,0])
    enc=base64.b64decode((relay/f"chunk{chunk}.b64").read_text().strip())
    plain=decrypt(enc,key,nonce); pos=0
    while pos < len(plain):
        ...
        target.parent.mkdir(parents=True,exist_ok=True); target.write_bytes(data); writes += 1
print("Quick Print 1.5.1 source writes:",writes)
if writes != 120: raise RuntimeError(f"unexpected source file count: {writes}")
